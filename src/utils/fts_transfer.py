...
def do_transfer(context, src_dst_list, account_options, dry_run=False):
    ...
    job = fts3.new_job(transfers, verify_checksum=True, overwrite=True, metadata=f"User data subscription transfer Job by {account_type} {username}", retry=3, priority =3)
